...
class WRTD(WRTD_wrapper):
    ...
    def check_for_errors(self, status, cmd):
        ...
        if(status != self.WRTD_SUCCESS):
            self.wrtd_get_error(self.wrtd_p, byref(error_code), 256,
                                error_description)
            print("Error while executing command: " + str(cmd) +
                  " : " + str(error_description.value))
            os._exit(1)
    ...
    def set_rule(self, name, delay_ps, src_p, dst_p):
        ...
        status = self.wrtd_set_attr_string(self.wrtd_p, name,
            self.WRTD_ATTR_RULE_SOURCE, src_p)
        self.check_for_errors(status, 'set_rule')
        status = self.wrtd_set_attr_string(self.wrtd_p, name,
            self.WRTD_ATTR_RULE_DESTINATION, dst_p)
        self.check_for_errors(status, 'set_rule')
        status = self.wrtd_set_attr_tstamp(self.wrtd_p, name,
            self.WRTD_ATTR_RULE_DELAY, byref(ts))
        self.check_for_errors(status, 'set_rule')

    @encode_arguments
    def enable_rule(self, name):
        status = self.wrtd_set_attr_bool(self.wrtd_p, name,
            self.WRTD_ATTR_RULE_ENABLED, 1);
        self.check_for_errors(status, 'enable_rule')

    @encode_arguments
    def disable_rule(self, name):
        status = self.wrtd_set_attr_bool(self.wrtd_p, name,
            self.WRTD_ATTR_RULE_ENABLED, 0);
