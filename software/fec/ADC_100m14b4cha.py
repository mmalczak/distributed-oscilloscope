...
class ADC_100m14b4cha(ADC_Generic):
    ...
    """List of known voltage ranges to be used with the configuration
    option"""
    """ADC_CONF_CHN_RANGE"""
    ...
    """List of possible buffer types (options
    for ADC_CONF_100M14B4CHA_BUF_TYPE)"""
    ...
    """the ZIO buffer type in use"""
    ADC_CONF_100M14B4CHA_TRG_SW_EN = 1
    """software trigger enable/disable"""
    ADC_CONF_100M14B4CHA_ACQ_MSHOT_MAX = 2
    """Maximum size for a single shot in multi-shot mode
                                                 (in samples)"""
    ADC_CONF_100M14B4CHA_BUF_SIZE_KB = 3
    """it manually sets the buffer size but only for VMALLOC buffers"""
    ADC_CONF_100M14B4CHA_TRG_ALT_EN = 4
    """alternate trigger enable/disable"""
